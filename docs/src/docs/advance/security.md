...
Note that it is possible to bind Rudis to a single interface by setting the corresponding field in the `onedis_server` section of `config/unidb.toml`:
...
The legacy authentication method is enabled by editing `Rudis.toml` and setting `requirepass`. This password is then used by all clients.
