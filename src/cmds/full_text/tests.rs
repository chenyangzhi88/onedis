...
fn full_text_search_aggregate_profile_and_explain_parsers_cover_options() {
    ...
    let load_all =
        FtAggregate::parse_from_frame(frame(&["FT.AGGREGATE", "idx", "*", "LOAD", "*"])).unwrap();
    ...
    assert!(
        FtProfile::parse_from_frame(frame(&["FT.PROFILE", "idx", "BAD", "QUERY", "*"])).is_err()
    );
    assert!(
        FtProfile::parse_from_frame(frame(&["FT.PROFILE", "idx", "SEARCH", "BAD", "*"])).is_err()
    );
    ...
    assert!(
        FtExplain::parse_from_frame(frame(&["FT.EXPLAIN", "idx", "*", "DIALECT", "5"])).is_err()
    );
}
...
fn full_text_misc_parsers_cover_alias_cursor_dict_sug_syn_and_unsupported() {
    ...
    assert!(matches!(
        FtConfig::parse_from_frame(frame(&["FT.CONFIG", "SET", "DEFAULT_DIALECT", "4"])).unwrap(),
        FtConfig::Set { .. }
    ));
    ...
    assert!(FtCursor::parse_from_frame(frame(&["FT.CURSOR", "READ", "idx", "1", "BAD"])).is_err());
    ...
    assert!(
        FtSpellCheck::parse_from_frame(frame(&["FT.SPELLCHECK", "idx", "q", "TERMS", "BAD", "d"]))
            .is_err()
    );
...
fn full_text_parsers_reject_unsafe_counts_and_invalid_search_combinations() {
    ...
    let return_zero =
        FtSearch::parse_from_frame(frame(&["FT.SEARCH", "idx", "*", "RETURN", "0"])).unwrap();
    assert!(return_zero.options.no_content);
    assert!(return_zero.options.return_fields.unwrap().is_empty());
    assert!(FtSearch::parse_from_frame(frame(&["FT.SEARCH", "idx", "*", "INKEYS", "0"])).is_err());
    assert!(
        FtSearch::parse_from_frame(frame(&["FT.SEARCH", "idx", "*", "INFIELDS", "0"])).is_err()
    );
    assert!(FtSearch::parse_from_frame(frame(&["FT.SEARCH", "idx", "*", "EXPLAINSCORE"])).is_err());
    assert!(
        FtSearch::parse_from_frame(frame(&["FT.SEARCH", "idx", "*", "SLOP", "4294967296",]))
            .is_err()
    );
    assert!(
...
fn full_text_profile_spell_synonym_and_hybrid_options_are_preserved() {
    let profile =
        FtProfile::parse_from_frame(frame(&["FT.PROFILE", "idx", "SEARCH", "QUERY", "*"])).unwrap();
    ...
    assert!(
        FtSpellCheck::parse_from_frame(frame(&["FT.SPELLCHECK", "idx", "helo", "DISTANCE", "5",]))
            .is_err()
    );
    ...
    assert_eq!(hybrid.options.search.params.get("vec").unwrap(), b"[1,0]");
    assert_eq!(
