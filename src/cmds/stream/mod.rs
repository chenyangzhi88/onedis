...
use crate::store::db::StreamEntry;
...
mod tests {
    ...
    async fn stream_wrappers_parse_apply_and_async_error_frames_cover_edges() {
        ...
        assert!(
            Xpending::parse_from_frame(frame(&["XPENDING", "s", "g", "bad", "+", "10"])).is_err()
        );
        ...
        assert!(
            Xread::parse_from_frame(frame(&["XREAD", "COUNT", "bad", "STREAMS", "s", "0-0"]))
                .is_err()
        );
        ...
        assert!(
            Xclaim::parse_from_frame(frame(&["XCLAIM", "s", "g", "c2", "bad", "1-0"])).is_err()
        );
        assert!(Xclaim::parse_from_frame(frame(&["XCLAIM", "s", "g", "c2", "0", "bad"])).is_err());
        assert!(
            !assert_array(
                Xclaim::parse_from_frame(frame(&[
                    "XCLAIM",
                    "s",
                    "g",
                    "c2",
        ...
        assert!(
            Xautoclaim::parse_from_frame(frame(&["XAUTOCLAIM", "s", "g", "c3", "0", "bad",]))
                .is_err()
        );
