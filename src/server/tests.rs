...
async fn test_handler(databases: usize, requirepass: Option<&str>) -> (Handler, TcpStream) {
    ...
    let session_manager = Arc::new(SessionManager::with_default_password(requirepass));
...
#[test]
fn blocking_commands_reject_non_finite_timeouts() {
    for args in [
        &["BLPOP", "list", "NaN"][..],
        &["BRPOPLPUSH", "src", "dst", "inf"][..],
        &["BLMOVE", "src", "dst", "LEFT", "RIGHT", "-inf"][..],
        &["BLMPOP", "NaN", "1", "list", "LEFT"][..],
        &["BZPOPMIN", "zset", "inf"][..],
        &["BZMPOP", "NaN", "1", "zset", "MIN"][..],
    ] {
        let frame = Frame::Array(
            args.iter()
                .map(|arg| Frame::bulk_string((*arg).to_string()))
        ...
        let error = Command::parse_from_frame(frame)
            .err()
            .expect("non-finite timeout must be rejected");
...
fn handler_fast_paths_acl_pubsub_and_monitor_cover_private_routes() {
    ...
    assert!(matches!(
        handler.apply_pubsub_introspection(&["CHANNELS".to_string(), "other*".to_string()]),
    ...
    assert!(matches!(
        ...
        Frame::Error(message) if message.contains("syntax")
    ));
...
#[test]
fn handler_accepts_redis_cli_pipe_trailer_and_binary_echo_marker() {
    let rt = tokio::runtime::Runtime::new().unwrap();
    let (mut handler, mut client) = rt.block_on(test_handler(1, None));
    rt.block_on(async {
        let client_io = async {
            let marker = [0xff, 0x00, 0x80, b'x'];
            ...
            .await
            .expect("pipeline replies timed out")
            .unwrap();
...
#[test]
fn handler_rejects_empty_mget_and_subscribe_restricts_mode_and_quit_closes_pipeline() {
    let rt = tokio::runtime::Runtime::new().unwrap();
    let (mut handler, mut client) = rt.block_on(test_handler(1, None));
    rt.block_on(async {
        let client_io = async {
            ...
            tokio::time::timeout(std::time::Duration::from_secs(2), async {
                ...
                while !response.ends_with(b"+OK\r\n") {
                    ...
                    assert!(read > 0, "connection closed before QUIT response");
                    response.extend_from_slice(&chunk[..read]);
                }
            })
            .await
            .expect("pipeline response timed out");
            let response = String::from_utf8_lossy(&response);
            assert!(response.contains("wrong number of arguments for 'mget' command"));
            assert!(response.contains("wrong number of arguments for 'subscribe' command"));
            assert!(response.contains("$9\r\nsubscribe\r\n$4\r\nnews\r\n:1\r\n"));
            assert!(response.contains("only (P|S)SUBSCRIBE"));
            assert_eq!(response.matches("+PONG\r\n").count(), 1);
            assert!(response.ends_with("+OK\r\n"));

            let mut unexpected = [0_u8; 32];
            assert!(
                tokio::time::timeout(
                    std::time::Duration::from_millis(100),
                    client.read(&mut unexpected),
                )
                .await
                .is_err(),
                "pipelined command after QUIT unexpectedly received a reply"
            );
        };
        let (_, ()) = tokio::join!(handler.handle(), client_io);
    });
...
#[test]
fn fast_paths_are_acl_checked_and_visible_to_monitor() {
    let rt = tokio::runtime::Runtime::new().unwrap();
    let (mut handler, _) = rt.block_on(test_handler(1, None));
    let (mut monitor_client, _monitor_connection) = rt.block_on(async {
        let (monitor_server, monitor_client) = connected_streams().await;
        let mut monitor_connection = Connection::new(monitor_server);
        handler
            .session_manager
            .add_monitor(usize::MAX, monitor_connection.shared_writer());
        (monitor_client, monitor_connection)
    });
    ...
    let mut monitor_output = [0_u8; 512];
    let read = rt
        .block_on(async {
            tokio::time::timeout(
        ...
        .expect("monitor did not receive fast-path commands")
        .unwrap();
    let monitor_output = String::from_utf8_lossy(&monitor_output[..read]);
    assert!(monitor_output.contains("\"ping\"") || monitor_output.contains("\"set\""));
}

#[test]
fn monitor_connection_receives_its_own_commands_after_their_replies() {
    let rt = tokio::runtime::Runtime::new().unwrap();
    let (mut handler, mut client) = rt.block_on(test_handler(1, None));
    rt.block_on(async {
        let client_io = async {
            ...
            tokio::time::timeout(std::time::Duration::from_secs(2), async {
                ...
                loop {
                    let read = client.read(&mut chunk).await.unwrap();
                    assert!(read > 0, "monitor connection closed before all replies");
                    response.extend_from_slice(&chunk[..read]);
                    let text = String::from_utf8_lossy(&response);
                    if text.matches("+OK\r\n").count() >= 2 && text.contains("\"echo\"") {
                        break;
                    }
                }
            })
            .await
            .expect("monitor pipeline response timed out");

            let response = String::from_utf8_lossy(&response);
            let pong = response.find("+PONG\r\n").unwrap();
            let ping_trace = response.find("\"ping\"").unwrap();
            let echo_reply = response.find("$2\r\nhi\r\n").unwrap();
            let echo_trace = response.find("\"echo\"").unwrap();
            assert!(pong < ping_trace);
            assert!(ping_trace < echo_reply);
            assert!(echo_reply < echo_trace);
            assert!(response.ends_with("+OK\r\n"));
        };
        let (_, ()) = tokio::join!(handler.handle(), client_io);
    });
...
#[test]
fn requirepass_cannot_be_bypassed_through_default_acl_user() {
    let rt = tokio::runtime::Runtime::new().unwrap();
    let (mut handler, _) = rt.block_on(test_handler(1, Some("secret")));

    assert!(handler.login(Some("default"), "wrong").is_err());
    assert!(!handler.session.get_certification());
    assert!(handler.login(Some("default"), "secret").is_ok());
    assert!(handler.session.get_certification());
}

#[test]
fn disconnected_blocking_client_releases_its_session() {
    let rt = tokio::runtime::Runtime::new().unwrap();
    let (sessions, executor) = rt.block_on(async {
        let (mut handler, mut client) = test_handler(1, None).await;
        let sessions = handler.session_manager.clone();
        let executor = handler.command_executor.clone();
        assert_eq!(sessions.get_connection_count(), 1);
        ...
        tokio::time::timeout(std::time::Duration::from_secs(2), task)
            .await
            .expect("blocking handler retained a disconnected client")
            .unwrap();
        assert_eq!(sessions.get_connection_count(), 0);
        (sessions, executor)
    });
    assert_eq!(sessions.get_connection_count(), 0);
    drop(executor);
...
fn borrowed_fast_paths_cover_guards_and_error_branches() {
    ...
    let (mut auth_handler, _) = rt.block_on(test_handler(1, Some("secret")));
    assert!(
        rt.block_on(auth_handler.try_handle_borrowed_fast_batch(b"*2\r\n$3\r\nGET\r\n$1\r\nk\r\n"))
            .is_none()
    );
...
fn blocking_handlers_pop_ready_values_and_timeout_when_empty() {
    let rt = tokio::runtime::Runtime::new().unwrap();
    let (mut handler, _client) = rt.block_on(test_handler(1, None));
    ...
    let bytes = rt
        .block_on(handler.apply_blocking_list_command(command(&["brpoplpush", "l", "dst", "0.01"])))
        .unwrap();
    assert!(String::from_utf8_lossy(&bytes).contains("$1\r\nb"));
    ...
    let bytes = rt
        .block_on(handler.apply_blocking_zset_command(command(&["bzpopmax", "missing", "0.001"])))
        .unwrap();
    assert_eq!(bytes, Frame::Null.as_bytes());
...
fn blocking_once_handlers_cover_remaining_list_zset_and_stream_group_shapes() {
    ...
    .unwrap();
    let frame = rt
        .block_on(handler.try_blocking_zset_command_once(&command(&["bzpopmax", "myzset", "0.01"])))
        .unwrap()
        .unwrap();
    assert!(matches!(
        frame,
...
fn transaction_async_dirty_watch_and_state_errors_are_reported() {
    ...
    handler.queue_transaction_frame(
        Frame::Array(vec![
            Frame::bulk_string("set"),
            Frame::bulk_string("acl-denied"),
            Frame::bulk_string("v"),
        ]),
        &mut denied,
    );
    assert!(String::from_utf8_lossy(&denied).contains("NOPERM"));
    assert!(matches!(
        rt.block_on(handler.execute_transaction_async()).unwrap(),
    ...
    handler
        ...
        .unwrap();
    handler.start_transaction().unwrap();
    let mut queued_before_acl_change = Vec::new();
    handler.queue_transaction_frame(
        Frame::Array(vec![
            Frame::bulk_string("set"),
            Frame::bulk_string("acl-revalidated"),
            Frame::bulk_string("v"),
        ]),
        &mut queued_before_acl_change,
    );
    assert_eq!(queued_before_acl_change, b"+QUEUED\r\n");
    handler
        .session_manager
        .acl_setuser("default", &["-set".to_string()])
        .unwrap();
    assert!(matches!(
        rt.block_on(handler.execute_transaction_async()).unwrap(),
        Frame::Error(message) if message.contains("no permissions")
    ));
    assert!(
        !handler
            .get_session()
