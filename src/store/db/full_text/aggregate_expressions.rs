use super::*;
pub(super) fn eval_fulltext_aggregate_filter(
    expression: &str,
...
pub(super) fn eval_fulltext_aggregate_expression(
    expression: &str,
    row: &FullTextAggregateRow,
...
pub(super) fn eval_fulltext_aggregate_function(
    expression: &str,
    row: &FullTextAggregateRow,
...
pub(super) fn find_fulltext_aggregate_operator(expression: &str, op: &str) -> Option<usize> {
    find_fulltext_aggregate_top_level(expression, &[op])
}

pub(super) fn find_fulltext_aggregate_arithmetic(
    expression: &str,
    ops: &[char],
) -> Option<(usize, char)> {
...
pub(super) fn find_fulltext_aggregate_top_level(expression: &str, ops: &[&str]) -> Option<usize> {
    let mut depth = 0usize;
    let mut quote = None;
...
pub(super) fn trim_wrapping_parens(mut expression: &str) -> &str {
    loop {
...
pub(super) fn parse_quoted_fulltext_aggregate_string(expression: &str) -> Option<String> {
