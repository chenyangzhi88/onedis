use super::*;
pub(super) fn compare_fulltext_sort_keys(
...
pub(super) fn format_fulltext_score(score: f32) -> String {
    if score.fract() == 0.0 {
        format!("{score:.1}")
    } else {
        score.to_string()
...
pub(super) fn json_index_strings(value: &serde_json::Value) -> Vec<String> {
    match value {
...
pub(super) fn json_index_tag_values(value: &serde_json::Value) -> Vec<String> {
    match value {
        serde_json::Value::String(value) => vec![value.clone()],
        serde_json::Value::Number(value) => vec![value.to_string()],
        serde_json::Value::Bool(value) => vec![value.to_string()],
...
pub(super) fn json_index_numeric_values(value: &serde_json::Value) -> Vec<String> {
    match value {
...
pub(super) fn parse_fulltext_json_path(path: &str) -> Result<Vec<FullTextJsonPathToken>, Error> {
    if path == "$" || path == "." {
        return Ok(Vec::new());
    }
...
pub(super) fn fulltext_json_path_values(
...
pub(super) fn fulltext_schema_frame(schema: &[FullTextFieldSchema]) -> Frame {
    Frame::Array(
        schema
            .iter()
            .map(|field| {
...
pub(super) fn fulltext_geoshape_coordinate_system_name(
...
pub(super) fn fulltext_vector_algorithm_name(algorithm: FullTextVectorAlgorithm) -> &'static str {
    match algorithm {
...
pub(super) fn fulltext_vector_options_frame(options: &FullTextVectorOptions) -> Frame {
