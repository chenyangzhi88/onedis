...
pub(super) fn split_tag_values(raw: &str) -> Vec<String> {
...
pub(super) fn unescape_query_token(raw: &str) -> String {
...
pub(super) fn parse_numeric_bound(raw: &str) -> Result<FullTextNumericBound, Error> {
...
pub(super) fn numeric_bound_to_tantivy(
    field: Field,
    bound: FullTextNumericBound,
    lower: bool,
) -> Bound<Term> {
    match (bound, lower) {
...
pub(super) fn search_bound_to_tantivy(
    field: Field,
...
pub(super) fn parse_f64_token(raw: &str, error: &str) -> Result<f64, Error> {
    raw.parse::<f64>()
        .map_err(|_| Error::msg(error.to_string()))
}

pub(super) fn validate_fulltext_create(options: &FullTextCreateOptions) -> Result<(), Error> {
    ...
    for field in &options.schema {
        ...
        validate_fulltext_identifier(field.attribute_name(), "field")?;
...
pub(super) fn validate_fulltext_identifier(value: &str, kind: &str) -> Result<(), Error> {
    if value.is_empty()
        || value.len() > 1_024
        || value.contains('\0')
        || value.starts_with("__onedis_")
    {
        Err(Error::msg(format!("ERR invalid fulltext {kind} name")))
    } else {
        Ok(())
    }
}

pub(super) fn validate_fulltext_field(field: &FullTextFieldSchema) -> Result<(), Error> {
    match field.kind {
...
pub(super) fn validate_fulltext_vector_options(
...
pub(super) fn fulltext_source_type_name(source_type: FullTextSourceType) -> &'static str {
    match source_type {
...
pub(super) fn fulltext_state_name(state: FullTextIndexState) -> &'static str {
    match state {
