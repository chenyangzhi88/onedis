    ...
    fn vector_parsing_distance_and_query_params_cover_success_and_errors() {
        ...
        assert!(
            (fulltext_vector_distance("L2", &[1.0, 2.0], &[3.0, 4.0]).unwrap() - 8.0).abs() < 1e-6
        );
        assert!(
    ...
    fn aggregate_expressions_reducers_sort_and_frames_cover_success_and_errors() {
        ...
        assert!(
            fulltext_aggregate_value_to_number(&FullTextAggregateValue::List(Vec::new())).is_err()
        );
        assert_eq!(
        ...
        let grouped =
            fulltext_aggregate_group(rows, &["@category".to_string()], &reducers).unwrap();
        let books = grouped
            .iter()
    ...
    fn geo_geoshape_numeric_filter_and_ast_matching_cover_edges() {
        ...
        let schema_meta = meta(vec![
            ...
            field("loc", FullTextFieldKind::Geo),
            {
                ...
                field.options.geoshape_coordinate_system =
                    Some(FullTextGeoShapeCoordinateSystem::Flat);
                field
            },
        ]);
        let fields = vec![
        ...
        assert!(
            fulltext_eval_ast_against_fields(
                &FullTextQueryAst::Optional(Box::new(FullTextQueryAst::Text(
                    "missing".to_string()
                ))),
                &fields,
                &schema_meta,
    ...
    fn runtime_indexes_searches_deletes_synonyms_and_registry_paths() {
        ...
        let mut runtime = FullTextRuntime::new(store.clone(), 0, "idx", &meta).unwrap();
        ...
        let runtime = FullTextRuntime::new(store, 0, "idx2", &meta).unwrap();
