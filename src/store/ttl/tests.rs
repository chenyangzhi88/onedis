...
fn nul_key_owner_encoding_cannot_overlap_plain_key_range() {
    ...
    let nested_start = sub_key_range_start(encoding, 0, &HASH_FIELD_NS, &nested_key, 2);
...
fn ttl_manager_add_remove_db_rebuild_and_parse_edges_work() {
    ...
    manager.add(0, 0, "ignored".to_string()).unwrap();
    assert_eq!(manager.index_size().unwrap(), 0);

    manager.add(100, 0, "db0-a".to_string()).unwrap();
    manager.add(200, 0, "db0-b".to_string()).unwrap();
    manager.add(150, 1, "db1-a".to_string()).unwrap();
    assert_eq!(manager.index_size().unwrap(), 3);

    let mut db0_deletes = WriteBatch::new();
    manager.remove_db_to_batch(&mut db0_deletes, 0).unwrap();
    assert_eq!(db0_deletes.count(), 2);
    db0_store.write_batch(&db0_deletes).unwrap();
    assert_eq!(manager.index_size().unwrap(), 1);
    ...
    db1_store.write_batch(&known).unwrap();
    assert_eq!(manager.index_size().unwrap(), 0);
    ...
    *invalid_utf8.last_mut().unwrap() = 0xff;
    ...
    db0_store.put_raw(&owner_key, b"owner").unwrap();

    let vc = VersionCounter::new();
    manager.rebuild_from_store(2, &vc).unwrap();
...
async fn ttl_manager_async_remove_rebuild_index_size_and_empty_sweep_work() {
    ...
    manager.add(100, 0, "a".to_string()).unwrap();
    manager.add(200, 0, "b".to_string()).unwrap();
    assert_eq!(manager.index_size_async().await.unwrap(), 2);

    let mut deletes = WriteBatch::new();
    manager
        .remove_db_to_batch_async(&mut deletes, 0)
        .await
        .unwrap();
    assert_eq!(deletes.count(), 2);
    db0_store.write_batch(&deletes).unwrap();
    assert_eq!(manager.index_size_async().await.unwrap(), 0);

    let mut owner_key = crate::store::db::version_owner_prefix(0);
    owner_key.extend_from_slice(&30u64.to_be_bytes());
    db0_store.put_raw(&owner_key, b"owner").unwrap();
    let vc = VersionCounter::new();
    manager.rebuild_from_store_async(1, &vc).await.unwrap();
...
async fn ttl_expire_observer_runs_only_after_committed_delete() {
    ...
    manager.set_expire_observer(Arc::new(move |db_index, key, type_tag, version| {
        observed_from_hook
            .lock()
            .unwrap()
            .push((db_index, key.to_string(), type_tag, version));
    }));
    ...
    db_store
        ...
        .unwrap();
    manager
        .add(expired, 0, "vector-expired".to_string())
        .unwrap();

    assert!(!manager.sweep_once_async().await);
    assert!(
        db_store
            .get_raw(&main_key(0, "vector-expired"))
            .unwrap()
            .is_none()
    );
    assert_eq!(
...
async fn ttl_scan_is_deadline_bounded_and_rotates_between_databases() {
    ...
    for key in ["db0-a", "db0-b"] {
        db0_store
            .put_raw(&main_key(0, key), &regular_meta(expired, 1, TYPE_STRING))
            .unwrap();
        manager.add(expired, 0, key.to_string()).unwrap();
    }
    db0_store
        .put_raw(
        ...
        .unwrap();
    manager.add(future, 0, "db0-future".to_string()).unwrap();
    db1_store
        .put_raw(
        ...
        .unwrap();
    manager.add(expired, 1, "db1-expired".to_string()).unwrap();

    let first = manager.scan_expired_batch_async(expired, 1).await.unwrap();
    assert_eq!(first.len(), 1);
    assert_eq!(first[0].db_index, 0);

    assert!(manager.sweep_once_async().await);
    assert_eq!(
        db1_store.get_raw(&main_key(1, "db1-expired")).unwrap(),
        None
    );
    assert_eq!(
        db0_store.get_raw(&main_key(0, "db0-future")).unwrap(),
...
async fn ttl_sweeper_drains_backlog_without_waiting_between_full_batches() {
    ...
    tokio::time::timeout(Duration::from_millis(250), async {
        while manager.index_size().unwrap() != 0 {
            tokio::task::yield_now().await;
        }
    })
    .await
    .expect("a full TTL batch must schedule its backlog without another interval tick");
    manager.shutdown();
    sweeper.await.unwrap();

    assert_eq!(manager.index_size().unwrap(), 0);
    for key in ["backlog-a", "backlog-b", "backlog-c"] {
        assert_eq!(db0_store.get_raw(&main_key(0, key)).unwrap(), None);
    }
}
...
async fn ttl_sweep_uses_the_persisted_legacy_key_layout() {
    ...
    let db0_store = store.for_db_index(0);
    db0_store
        .put_raw(b"\x80m\x00layout", b"db-prefixed-v1")
        .unwrap();
    ...
    assert_eq!(db0_store.get_raw(&legacy_main_key).unwrap(), None);
    assert_eq!(manager.index_size().unwrap(), 0);
}
...
async fn ttl_sweep_of_plain_key_preserves_a_distinct_nul_key() {
    ...
    assert!(db.expire("a".to_string(), 1).unwrap());
    tokio::time::sleep(Duration::from_millis(5)).await;
    assert!(!manager.sweep_once_async().await);
...
async fn ttl_sweep_deletes_valid_expired_keys_and_skips_not_found_stale_and_hook_rejected() {
    ...
    db0_store
        .put_raw(&main_key(0, "valid"), &regular_meta(expired, 1, TYPE_HASH))
        .unwrap();
    manager.add(expired, 0, "valid".to_string()).unwrap();

    manager.add(expired, 0, "missing".to_string()).unwrap();

    db0_store
        .put_raw(&main_key(0, "stale"), &regular_meta(future, 2, TYPE_SET))
        .unwrap();
    manager.add(expired, 0, "stale".to_string()).unwrap();

    db0_store
        .put_raw(&main_key(0, "bad-meta"), b"short")
        .unwrap();
    manager.add(expired, 0, "bad-meta".to_string()).unwrap();

    db0_store
        .put_raw(&main_key(0, "hooked"), &regular_meta(expired, 3, TYPE_LIST))
        .unwrap();
    manager.add(expired, 0, "hooked".to_string()).unwrap();
    ...
    assert!(!manager.sweep_once_async().await);
    assert_eq!(db0_store.get_raw(&main_key(0, "valid")).unwrap(), None);
    assert_eq!(db0_store.get_raw(&main_key(0, "missing")).unwrap(), None);
    assert_eq!(
        db0_store.get_raw(&main_key(0, "stale")).unwrap(),
        Some(regular_meta(future, 2, TYPE_SET))
    );
    assert_eq!(
        db0_store.get_raw(&main_key(0, "bad-meta")).unwrap(),
        Some(b"short".to_vec())
    );
    assert_eq!(
        db0_store.get_raw(&main_key(0, "hooked")).unwrap(),
        Some(regular_meta(expired, 3, TYPE_LIST))
    );
    assert_eq!(db0_store.get_raw(b"hook:called").unwrap(), None);
...
async fn ttl_async_sweep_honors_batch_size_and_defers_json_cleanup_to_compaction() {
    ...
    for key in ["json-a", "json-b"] {
        db0_store
            .put_raw(&main_key(0, key), &regular_meta(expired, 8, TYPE_JSON))
            .unwrap();
        let mut node_key = json_node_prefix(0, key, 8);
        node_key.extend_from_slice(b":node");
        db0_store.put_raw(&node_key, b"node").unwrap();
        manager.add(expired, 0, key.to_string()).unwrap();
    }
...
async fn ttl_async_sweep_does_not_delete_a_concurrently_replaced_value() {
    ...
    db0_store
        .put_raw(&main_key(0, "raced"), &regular_meta(expired, 1, TYPE_HASH))
        .unwrap();
    manager.add(expired, 0, "raced".to_string()).unwrap();
    ...
    assert!(!manager.sweep_once_async().await);
    assert_eq!(
        db0_store.get_raw(&main_key(0, "raced")).unwrap(),
