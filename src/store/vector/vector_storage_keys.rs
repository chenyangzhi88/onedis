fn vector_prefix(
    ...
    layout.sub_key_range_start_bytes(db_index, ns, index.as_bytes(), version)
}
...
fn vector_doc_prefix(
    ...
    vector_prefix(layout, db_index, &VECTOR_DOC_NAMESPACE, index, version)
}
...
fn vector_doc_key(
    ...
    let mut key = vector_doc_prefix(layout, db_index, index, version);
    key.extend_from_slice(id.as_bytes());
    key
}

fn vector_segment_prefix(
    layout: KeyEncodingLayout,
    db_index: u16,
    index: &str,
    version: u64,
) -> Vec<u8> {
    vector_prefix(layout, db_index, &VECTOR_SEGMENT_NAMESPACE, index, version)
}

fn vector_segment_key(
    layout: KeyEncodingLayout,
    db_index: u16,
    index: &str,
    version: u64,
    ...
    let mut key = vector_segment_prefix(layout, db_index, index, version);
...
fn vector_tag_key(
    ...
    let mut key = vector_prefix(layout, db_index, &VECTOR_TAG_NAMESPACE, index, version);
...
fn vector_numeric_key(
    ...
    let mut key = vector_prefix(layout, db_index, &VECTOR_NUMERIC_NAMESPACE, index, version);
    append_len_prefixed(&mut key, field.as_bytes());
...
fn vector_numeric_field_prefix(
    layout: KeyEncodingLayout,
    db_index: u16,
    index: &str,
    version: u64,
