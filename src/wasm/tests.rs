...
fn add_i64_module() -> Vec<u8> {
    vec![
        ...
        0x01, 0x7e, 0x03, 0x02, 0x01, 0x00, 0x07, 0x07, 0x01, 0x03, 0x61, 0x64, 0x64, 0x00, 0x00,
...
fn wasm_limits_and_import_validation_cover_resource_edges() {
    ...
    let allowed_import = vec![
        ...
        0x14, 0x01, 0x06, b'o', b'n', b'e', b'd', b'i', b's', 0x09, b'r', b'e', b'd', b'i', b's',
        b'_', b'g', b'e', b't', 0x00, 0x00,
    ];
    registry.load("allowed_import", &allowed_import).unwrap();

    let bad_import = vec![
        0x00, 0x61, 0x73, 0x6d, 0x01, 0x00, 0x00, 0x00, 0x01, 0x04, 0x01, 0x60, 0x00, 0x00, 0x02,
        0x09, 0x01, 0x03, b'b', b'a', b'd', 0x01, b'x', 0x00, 0x00,
    ];
