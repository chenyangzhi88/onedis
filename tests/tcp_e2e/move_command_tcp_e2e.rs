...
mod tests {
    ...
    fn test_move_nonexistent_key() {
        ...
        // 尝试移动一个不存在的键
        let moved: i32 = crate::support::move_key(&mut con, "nonexistent-move-key", 1).unwrap();
        assert_eq!(moved, 0);
    }
    ...
    fn test_move_to_same_database() {
        ...
        // 尝试将键移动到同一个数据库
        let moved: i32 = crate::support::move_key(&mut con, "same-db-move-key", 0).unwrap();
    ...
    fn test_move_key_already_exists_in_target() {
        ...
        // 切换回数据库0并尝试移动键
        let _: () = crate::support::select_db(&mut con, 0).unwrap();
        let moved: i32 = crate::support::move_key(&mut con, "source-key", 1).unwrap();
    ...
    fn test_move_invalid_database_index() {
        ...
        let result = crate::support::move_key(&mut con, "invalid-db-key", 100);
